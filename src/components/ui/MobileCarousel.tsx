    ...
    return (
        <div className="relative group -mx-8 w-[calc(100%+4rem)] md:mx-0 md:w-full">
            <div
                ...
                className="flex overflow-x-auto snap-x snap-mandatory no-scrollbar md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 md:gap-6 lg:gap-8 gap-4 w-full py-12 -my-12 px-8 md:px-0 items-stretch"
                style={{ touchAction: 'pan-x' }}
            >
                {React.Children.map(children, (child) => (
                    <div className="w-[calc(50vw-24px)] sm:w-[320px] shrink-0 snap-center md:w-auto md:shrink flex items-stretch">
                        {child}
                    </div>
                ))}
            </div>
