 ...
 *
 * This component used to fetch the list itself and render a full-screen
 * spinner until it arrived. Because that spinner was the entire server
 * response, `/fr/journal` shipped HTML containing **zero** links to any of the
 * 17 published articles — they were reachable only from the sitemap, which
 * makes them orphans with no internal PageRank and no crawl path from the
 * homepage. Seeding the list on the server restores real `<a href>`s in the
 * initial markup.
 */
export default function JournalPageClient({
    ...
    useEffect(() => {
        // Server already provided the list; refetching would repaint the same
        // articles and undo the point of rendering them on the server.
    ...
    // Explicit type guard rather than `.filter(Boolean)`: that does not narrow
    // `string | undefined` down to `string`, and `category` became optional
    // when this component started accepting the server's post shape.
    const categories = [
