...
export default function ReturnsPage() {
    ...
    return (
        <div className="min-h-screen bg-ink text-sand pt-20 grain-overlay">
            {/* Hero */}
            <section className="bg-ink pt-16 pb-16 px-8 relative overflow-hidden">
                ...
                <div className="max-w-[1200px] mx-auto relative z-10">
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
                        ...
                        <h1 className="font-display text-6xl md:text-8xl uppercase text-sand leading-[0.85] mb-4">
                            ...
                            <span className="text-sand/30">{tx('& Remboursements', '& Refunds')}</span>
                        </h1>
                        <p className="text-sand/60 max-w-lg text-lg">
            ...
            <section className="max-w-[800px] mx-auto px-8 py-16">
                ...
                <motion.div
                    ...
                    className="bg-amber-500/10 border border-amber-500/25 rounded-2xl p-4 flex items-start gap-3 mb-10"
                >
                    ...
                    {[
                        ...
                        return (
                            <React.Fragment key={s.id}>
                                <div className={`flex items-center gap-2 ${isActive ? 'text-gold' : 'text-sand/30'}`}>
                                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-black ${isActive ? 'bg-gold text-ink' : 'bg-sand/10 text-sand/40'}`}>
                                        {i + 1}
                                    </div>
                                    <span className="text-[9px] font-bold uppercase tracking-widest hidden sm:block">{s.label}</span>
                                </div>
                                {i < 3 && <div className={`w-8 h-px ${isActive ? 'bg-gold' : 'bg-sand/15'}`} />}
                            </React.Fragment>
                        );
                    })}
                </div>
                ...
                {/* Forms */}
                <div className="bg-sand rounded-[32px] border border-ink/10 p-8 shadow-sm text-ink">
                            ...
                            <motion.div key="select" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                                ...
                                <div className="mb-6">
                                    ...
                                    <input
                                        ...
                                        className="w-full border-2 border-ink/15 bg-ink/5 rounded-xl px-4 py-3 text-sm text-ink placeholder:text-ink/30 focus:border-gold outline-none transition-colors"
                                    />
                                </div>
                                <div className="mb-8">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        {[
                                            ...
                                            <button
                                                ...
                                                className={`p-4 rounded-2xl border-2 text-left transition-all ${refundMethod === m.id ? 'border-gold bg-gold/10' : 'border-ink/10 hover:border-ink/20'}`}
                                            >
